...
def _read_csv(path, dtype=None):
    ...
    df = lib.read_csv(path, dtype=dtype)
    logging.debug(f'{os.path.basename(path)} {df.shape}')
    return df
def concurrent_read(file_specs):
    results = {}
    with ThreadPoolExecutor(max_workers=min(MAX_THREADS, len(file_specs))) as executor:
        future_to_key = {executor.submit(_read_csv, path, dtype): key for key, path, dtype in file_specs}
        for fut in as_completed(future_to_key):
            key = future_to_key[fut]
            results[key] = fut.result()
    return results
def get_balance_data():
    pos_dtype = {'SK_ID_PREV': np.uint32, 'SK_ID_CURR': np.uint32, 'MONTHS_BALANCE': np.int32, 'SK_DPD': np.int32, 'SK_DPD_DEF': np.int32, 'CNT_INSTALMENT': np.float32, 'CNT_INSTALMENT_FUTURE': np.float32}
    install_dtype = {'SK_ID_PREV': np.uint32, 'SK_ID_CURR': np.uint32, 'NUM_INSTALMENT_NUMBER': np.int32, 'NUM_INSTALMENT_VERSION': np.float32, 'DAYS_INSTALMENT': np.float32, 'DAYS_ENTRY_PAYMENT': np.float32, 'AMT_INSTALMENT': np.float32, 'AMT_PAYMENT': np.float32}
    card_dtype = {'SK_ID_PREV': np.uint32, 'SK_ID_CURR': np.uint32, 'MONTHS_BALANCE': np.int16, 'AMT_CREDIT_LIMIT_ACTUAL': np.int32, 'CNT_DRAWINGS_CURRENT': np.int32, 'SK_DPD': np.int32, 'SK_DPD_DEF': np.int32, 'AMT_BALANCE': np.float32, 'AMT_DRAWINGS_ATM_CURRENT': np.float32, 'AMT_DRAWINGS_CURRENT': np.float32, 'AMT_DRAWINGS_OTHER_CURRENT': np.float32, 'AMT_DRAWINGS_POS_CURRENT': np.float32, 'AMT_INST_MIN_REGULARITY': np.float32, 'AMT_PAYMENT_CURRENT': np.float32, 'AMT_PAYMENT_TOTAL_CURRENT': np.float32, 'AMT_RECEIVABLE_PRINCIPAL': np.float32, 'AMT_RECIVABLE': np.float32, 'AMT_TOTAL_RECEIVABLE': np.float32, 'CNT_DRAWINGS_ATM_CURRENT': np.float32, 'CNT_DRAWINGS_OTHER_CURRENT': np.float32, 'CNT_DRAWINGS_POS_CURRENT': np.float32, 'CNT_INSTALMENT_MATURE_CUM': np.float32}
    file_specs = [('pos_bal', os.path.join(RAW_DATA_DIR, 'POS_CASH_balance.csv'), pos_dtype), ('install', os.path.join(RAW_DATA_DIR, 'installments_payments.csv'), install_dtype), ('card_bal', os.path.join(RAW_DATA_DIR, 'credit_card_balance.csv'), card_dtype)]
    res = concurrent_read(file_specs)
    return res['pos_bal'], res['install'], res['card_bal']
def data_loaded():
    file_specs = [('apps', os.path.join(RAW_DATA_DIR, 'application_train.csv'), None), ('prev', os.path.join(RAW_DATA_DIR, 'previous_application.csv'), None), ('bureau', os.path.join(RAW_DATA_DIR, 'bureau.csv'), None), ('bureau_bal', os.path.join(RAW_DATA_DIR, 'bureau_balance.csv'), None)]
    balance_data = get_balance_data()
    balance_keys = ['pos_bal', 'install', 'card_bal']
    balance_map = dict(zip(balance_keys, balance_data))
    res = concurrent_read(file_specs)
    res.update(balance_map)
    return res['apps'], res['prev'], res['bureau'], res['bureau_bal'], res['pos_bal'], res['install'], res['card_bal']
...
def get_bureau_bal_agg(bureau, bureau_bal):
    ...
    bureau_bal_agg_dict = {'SK_ID_CURR': ['count'], 'MONTHS_BALANCE': ['min', 'max', 'mean'], 'BUREAU_BAL_IS_DPD': ['mean', 'sum'], 'BUREAU_BAL_IS_DPD_OVER120': ['mean', 'sum']}
...
def get_pos_bal_agg(pos_bal):
    ...
    pos_bal_agg_dict = {'SK_ID_CURR': ['count'], 'MONTHS_BALANCE': ['min', 'mean', 'max'], 'SK_DPD': ['min', 'max', 'mean', 'sum'], 'CNT_INSTALMENT': ['min', 'max', 'mean', 'sum'], 'CNT_INSTALMENT_FUTURE': ['min', 'max', 'mean', 'sum'], 'POS_IS_DPD': ['mean', 'sum'], 'POS_IS_DPD_UNDER_120': ['mean', 'sum'], 'POS_IS_DPD_OVER_120': ['mean', 'sum']}
    ...
    pos_bal_m20_agg_dict = {'SK_ID_CURR': ['count'], 'MONTHS_BALANCE': ['min', 'mean', 'max'], 'SK_DPD': ['min', 'max', 'mean', 'sum'], 'CNT_INSTALMENT': ['min', 'max', 'mean', 'sum'], 'CNT_INSTALMENT_FUTURE': ['min', 'max', 'mean', 'sum'], 'POS_IS_DPD': ['mean', 'sum'], 'POS_IS_DPD_UNDER_120': ['mean', 'sum'], 'POS_IS_DPD_OVER_120': ['mean', 'sum']}
...
def get_install_agg(install):
    ...
    install_agg_dict = {'SK_ID_CURR': ['count'], 'NUM_INSTALMENT_VERSION': ['nunique'], 'DAYS_ENTRY_PAYMENT': ['mean', 'max', 'sum'], 'DAYS_INSTALMENT': ['mean', 'max', 'sum'], 'AMT_INSTALMENT': ['mean', 'max', 'sum'], 'AMT_PAYMENT': ['mean', 'max', 'sum'], 'AMT_DIFF': ['mean', 'min', 'max', 'sum'], 'AMT_RATIO': ['mean', 'max'], 'SK_DPD': ['mean', 'min', 'max'], 'INS_IS_DPD': ['mean', 'sum'], 'INS_IS_DPD_UNDER_120': ['mean', 'sum'], 'INS_IS_DPD_OVER_120': ['mean', 'sum']}
    ...
    install_d365_agg_dict = {'SK_ID_CURR': ['count'], 'NUM_INSTALMENT_VERSION': ['nunique'], 'DAYS_ENTRY_PAYMENT': ['mean', 'max', 'sum'], 'DAYS_INSTALMENT': ['mean', 'max', 'sum'], 'AMT_INSTALMENT': ['mean', 'max', 'sum'], 'AMT_PAYMENT': ['mean', 'max', 'sum'], 'AMT_DIFF': ['mean', 'min', 'max', 'sum'], 'AMT_RATIO': ['mean', 'max'], 'SK_DPD': ['mean', 'min', 'max'], 'INS_IS_DPD': ['mean', 'sum'], 'INS_IS_DPD_UNDER_120': ['mean', 'sum'], 'INS_IS_DPD_OVER_120': ['mean', 'sum']}
...
def get_card_bal_agg(card_bal):
    ...
    card_bal_agg_dict = {'SK_ID_CURR': ['count'], 'AMT_BALANCE': ['max'], 'AMT_CREDIT_LIMIT_ACTUAL': ['max'], 'AMT_DRAWINGS_ATM_CURRENT': ['max', 'sum'], 'AMT_DRAWINGS_CURRENT': ['max', 'sum'], 'AMT_DRAWINGS_POS_CURRENT': ['max', 'sum'], 'AMT_INST_MIN_REGULARITY': ['max', 'mean'], 'AMT_PAYMENT_TOTAL_CURRENT': ['max', 'sum'], 'AMT_TOTAL_RECEIVABLE': ['max', 'mean'], 'CNT_DRAWINGS_ATM_CURRENT': ['max', 'sum'], 'CNT_DRAWINGS_CURRENT': ['max', 'mean', 'sum'], 'CNT_DRAWINGS_POS_CURRENT': ['mean'], 'SK_DPD': ['mean', 'max', 'sum'], 'BALANCE_LIMIT_RATIO': ['min', 'max'], 'DRAWING_LIMIT_RATIO': ['min', 'max'], 'CARD_IS_DPD': ['mean', 'sum'], 'CARD_IS_DPD_UNDER_120': ['mean', 'sum'], 'CARD_IS_DPD_OVER_120': ['mean', 'sum']}
